    ...
    <>
      <div className="flex md:flex-col lg:flex-row gap-8 items-center">
        <div className="flex flex-row items-center gap-2 md:w-155 lg:w-102 h-40 py-2 px-4 bg-white shadow-2xl rounded-2xl outline-2 outline-primary">
          <div className={`w-27 h-25 rounded-2xl flex justify-center items-center ${project?.level === 'basic' ? 'bg-green-500/25' : project?.level === 'intermediate' ? 'bg-yellow-500/25' : project?.level === 'advanced' ? 'bg-red-500/25' : 'bg-gray-500'}`}>
            <svg
              width="55"
              height="55"
              viewBox="0 0 50 50"
            ...
            >
              <path
                ...
                fill={project?.level === 'basic' ? '#16A34A' : project?.level === 'intermediate' ? '#EAB308' : project?.level === 'advanced' ? '#DC2626' : '#000000'}
              />
            </svg>
          </div>
